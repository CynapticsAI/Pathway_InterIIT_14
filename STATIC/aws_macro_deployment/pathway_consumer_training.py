...
rdkafka_consumer_settings = {
    "bootstrap.servers": os.getenv("KAFKA_BOOTSTRAP_SERVERS", "kafka:9090"),
...
def train_sector(df_fred: pd.DataFrame, sector: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        sentiment = "BULLISH" if last_prediction > 0.1 else "BEARISH" if last_prediction < -0.1 else "NEUTRAL"
        ...
        # CRITICAL FIX: Save only state_dict and model config, not the entire model
        model_data = {
            ...
            'last_data_date': str(X_engineered.index[-1])
        }
