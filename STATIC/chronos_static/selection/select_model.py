...
ohlc_stream = ohlc_stream.with_columns(
    timestamp=ohlc_stream.timestamp.dt.strptime(fmt="%Y-%m-%d %H:%M:%S%z"),
    symbol="TSLA",
)
...
ohlc_stream_2= ohlc_stream.with_columns(
    timestampf=ohlc_stream.timestamp - timedelta(minutes=1)
)
...
ohlc_stream= ohlc_stream.asof_join(
    ...
    ohlc_stream.timestamp== ohlc_stream_2.timestampf,
...
decision= comp.windowby(
    ...
    real_close= pw.reducers.argmax(pw.this.timestamp,pw.this.prev_close),
