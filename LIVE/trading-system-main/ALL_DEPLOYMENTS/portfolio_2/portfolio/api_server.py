...
app = FastAPI(title="Portfolio Optimizer API", version="3.0.0")
...
@app.post("/create_portfolio", tags=["1. Creator"])
async def create_portfolio(request: BaseRequest):
    """
    Creator: Filters master list constraints -> Generates FRESH optimized portfolio.
    """
    try:
...
@app.post("/rebalance_portfolio", tags=["2. Rebalancer"])
async def rebalance_portfolio(request: RebalanceRequest):
    """
    Rebalancer: Takes EXISTING portfolio -> Optimizes weights ONLY for those stocks.
    """
    try:
...
@app.post("/diversify_portfolio", tags=["3. Diversifier"])
async def diversify_portfolio(request: DiversifyRequest):
    """
    Diversifier: Takes EXISTING portfolio + Finds NEW candidates -> Optimizes Combined Universe.
    """
    try:
        pf = [item.dict() for item in request.current_portfolio]
